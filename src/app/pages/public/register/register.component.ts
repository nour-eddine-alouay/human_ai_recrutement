...
export class RegisterComponent implements OnInit{
  ...
  get pays(){
    return this.completeRegisterForm.controls['pays'];
  }
  get typeCompte(){
    return this.completeRegisterForm.controls['typeCompte'];
  }
  get profession(){
    return this.completeRegisterForm.controls['profession'];
  }
  get domaine(){
    return this.completeRegisterForm.controls['domaine'];
  }
  get site(){
    return this.completeRegisterForm.controls['site'];
  }


  register(){
    // candidat
    if(this.typeCompte.value === 'candidat'){
      const candidat : User = {
        nom: this.nom.value,
      ...
      this.auth.registerCandidat(candidat).subscribe( (result : any) => {
    ...
    // recruteur
      ...
      this.auth.registerRecruteur(recruteur).subscribe( (result : any) => {
        localStorage.setItem('user', JSON.stringify(result.user))
        localStorage.setItem('token', JSON.stringify(result.token))
  ...
  registerWithGoogle(){
    console.log('hey')
    this.auth.googleRegister().subscribe( (result : any) => {
      console.log(result)
    })
  }
  // isEmailUnique
  ...
  isEmailUnique(){
    this.auth.isEmailUnique(this.email.value).subscribe( (result : any) => {
      if(!result.isEmailUnique){
        this.isAlertMessage = true
        this.alertMessage = "Cet email existe déjà"
      }else{
