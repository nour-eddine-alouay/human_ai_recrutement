...
export class OffreDetailsComponent implements OnInit {
  ...
  getOffre(){
    ...
    this.offreService.getOffre(this.token.accessToken, this.id).subscribe( (result:any) =>{
      this.offre = result.data
      this.domaine = this.domaines.find( e => e.value==result.data.domaine)?.name
      this.offre.niveau_experience = this.experiences.find( e => e.value==result.data.niveau_experience)?.name
      this.offre.pays = this.paysList.find( p => p.code==result.data.pays)?.name
    })
  }
  ...
  modifier(id: number){
    ...
    this.router.navigate(['/recruteur/offre'], navigationExtras);
  }
  delete(){
    this.offreService.deleteOffre(this.token.accessToken, this.id);
