...
export class MonProfilCandidatComponent implements OnInit{
  ...
  getPersonalInfo(){
    ...
    .subscribe((result : any) => {
      if(result.success){
        ...
        const birthday = new Date(result.personal_information.date_naissance);
        ...
        this.PersonalInfo.pays = this.paysList.find(p => p.code == result.personal_information.pays)?.name
      }
    })
  }
  // getProfessionalInfo
  ...
  getProfessionalInfo(){
    ...
    .subscribe((result : any) => {
      if(result.success){
        ...
        this.ProfessionalInfo.domaine = this.domaines.find( d => d.value == this.ProfessionalInfo.domaine)?.name
        this.ProfessionalInfo.niveau_experience = this.experiences.find( d => d.value == this.ProfessionalInfo.niveau_experience)?.name
      }
    })
  }
  // get cv
  ...
  downloadPDF(url: string) {
    ...
    fetch(url)
      ...
      .then(blob => {
        ...
        // create a link element with the URL object as the href
        const link = document.createElement('a');
        link.href = url;
        link.download = this.CV?.url;
        // simulate a click event on the link element to trigger the download
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        // release the URL object from memory
