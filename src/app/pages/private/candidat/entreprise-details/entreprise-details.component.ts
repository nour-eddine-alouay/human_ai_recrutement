...
export class EntrepriseDetailsComponent implements OnInit{
  ...
  getEntrepriseDetails(){
    ...
    this.offreService.getEntrepriseDetails(this.token.accessToken, this.id).subscribe( (result:any) =>{
      console.log(result)
      this.entreprise = result.entreprise
      this.admin = result.admin
      // images
      this.entreprise_image_url = result.entreprise_image_url
      this.admin_image_url = result.admin_image_url
      // video
      this.video_url = result.video_url
      // pays et domaine conversion
      this.pays_name = this.paysList.find( p => p.code==result.entreprise.pays)?.name
      this.domaine_name = this.domaines.find(d => d.value == result.entreprise.domaine)?.name
      // valeurs humaines
      ...
      // offres récentes
      this.getOffresRecentes()
    })
  }

  offresRecentes !: any
  async getOffresRecentes(){
    this.offreService.getOffresRecentes(this.token.accessToken, this.entreprise.user_id).subscribe((result:any)=>{
      this.offresRecentes = result.offres
      console.log(result)
    })
  }
