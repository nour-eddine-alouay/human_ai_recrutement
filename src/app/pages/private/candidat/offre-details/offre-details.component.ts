...
export class OffreDetailsComponent implements OnInit{
  ...
  getOffre(){
    ...
    this.offreService.getOffre(this.token.accessToken, this.id).subscribe( (result:any) =>{
      console.log(result)
      this.offre = result.offre
      this.entreprise = result.entreprise
      this.image_url = result.image_url
      this.domaine = this.domaines.find( d => d.value==result.offre.domaine)?.name
      this.offre.niveau_experience = this.experiences.find( e => e.value==result.offre.niveau_experience)?.name
      this.pays_name = this.paysList.find( p => p.code==result.offre.pays)?.name
      this.getOffresSimilaire()
    })
  }
  ...
  offresSimilaires !: any
  async getOffresSimilaire(){
    console.log(this.offre?.domaine)
    this.offreService.getOffresSimilaires(this.token.accessToken, this.id, this.offre.domaine).subscribe((result:any)=>{
      this.offresSimilaires = result.offres
      console.log("offresSimilaires",result)
    })
  }
  ...
  voirOffreDetails(id: number): void {
    ...
    const newId = id
    const newState = { ...currentState, id: newId }
